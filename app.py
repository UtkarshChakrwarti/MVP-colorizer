...
application.config['MAX_IMAGES'] = 100
...
def process_image(file_path):
    ...
    output_path_siggraph17 = os.path.join(application.config['UPLOAD_FOLDER'], f'{filename}_siggraph17.png')
...
def remove_images():
    while True:
        ...
        images = glob.glob(os.path.join(application.config['UPLOAD_FOLDER'], '*.png'))
        if len(images) > application.config['MAX_IMAGES']:
            oldest_images = sorted(images, key=os.path.getctime)[:len(images) - application.config['MAX_IMAGES']]
...
@application.route('/uploads/<path:filename>', methods=['GET'])
